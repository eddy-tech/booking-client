...
export class FlightComponent implements OnInit {
  ...
  onBooking(id_flight: string, date_departiture: string | null, emailGuest: string, quantity: number, currency: string, luggages: number){
    if(!date_departiture) return;
    if(this.luggages < 0) return;
    const [year, month, day] = date_departiture?.split('-');
    const timestamp = new Date(+year, +month, +day).getTime();
    this.bookingService.saveBooking(id_flight, timestamp, emailGuest, quantity, currency, luggages).subscribe({
